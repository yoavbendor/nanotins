...
// PtpHeaderSpec reads byte-identically to the existing Gptp overlay (see test_wire_spec_ptp).
...
using PortIdentitySpec = WireSpec<
...
using ClockQualitySpec = WireSpec<
...
// prefix names it.
using SyncMsgSpec = WireSpec<_PtpHdr, embed<decltype("origin_"_fld), 34, TimestampSpec>>;
using FollowUpMsgSpec = WireSpec<_PtpHdr, embed<decltype("precise_origin_"_fld), 34, TimestampSpec>>;
using DelayReqMsgSpec = WireSpec<_PtpHdr, embed<decltype("origin_"_fld), 34, TimestampSpec>>;
...
using DelayRespMsgSpec = WireSpec<
    _PtpHdr,
...
using AnnounceMsgSpec = WireSpec<
    _PtpHdr,
...
using PtpTimestampBodySpec = WireSpec<embed<decltype("ts_"_fld), 0, TimestampSpec>>;

// Delay_Resp / Pdelay_Resp / Pdelay_Resp_Follow_Up: a Timestamp + the requesting PortIdentity (20 bytes).
using PtpTsPortBodySpec = WireSpec<
...
using PtpAnnounceBodySpec = WireSpec<
...
using PtpSignalingBodySpec = WireSpec<embed<decltype("target_"_fld), 0, PortIdentitySpec>>;
